...
RSpec.describe Log do
  ...
  before do
    allow(stack).to receive(:stacks).and_return([['branch1', 'branch3', 'branch4'], ['branch1', 'branch2']])
  end
  ...
  context 'when logging branches' do
    ...
    it 'returns the sorted branches' do
      expect(branches).to eq([
                               # Stack 1: b1 -> b2
                               # Stack 2: b1 -> b3 -> b4
                               { depth: 2, name: 'branch4', stacks: [0] },
                               { depth: 1, name: 'branch3', stacks: [0] },
                               { depth: 1, name: 'branch2', stacks: [1] },
